...
│   │   ├── pricing.ts            # the five line items (R8, R20, R23, R24)
│   │   ├── season.ts             # the calendar as a price input (R23)
│   │   ├── party.ts              # adults, children and the published child rate (R24)
│   │   ├── changes.ts            # what a re-plan changed, in one sentence (R19)
│   │   ├── scoring.ts            # destination fit score
│   │   ├── itinerary.ts          # base town, weekday rules, day blocks, legs (R7, R18, R21)
...
| refine-1 | `03-design.md` §4 S2: "Native controls: `<input type="date"` for dates. Native pickers only." | Text inputs that read and write `DD/MM/YYYY` (`parseDMY` / `formatDMY` in `domain/dates.ts`), with `DD/MM/YYYY` as a visible field hint and as the placeholder. ISO strings are still accepted on input, so a saved session and a pasted date both parse. | A native date input renders in the **browser's** locale, which in this environment is `MM/DD/YYYY` — the exact defect R12 (amended) names, and one no amount of CSS can fix. The trade is that we own parsing; it is 20 lines, pure, and unit-tested. |
| refine-1 | `03-design.md` §4 S2: the travellers field is labelled `Travellers`, hinted `Priced per adult traveller`. | `Adults` plus a new `Children` field with one age box per child (R24). The hint reads `Priced at the full adult fare. One room per two travellers, rounded up.` The adjust panel's field is `Adults` for the same reason. | R24 exists because "Priced per adult traveller" was a disclosure of a gap rather than a solution. A field labelled `Travellers` that silently means adults is the same defect with more words. |
| refine-1 | R24's example enters children aged 9 **and 12**, while the published rule prices children 2–11. | Children are counted exactly as entered (ages 0–17 accepted); a traveller aged 12 or over is priced at the **adult** fare, and the plan prints that sentence next to the rule. The summary bar still reads `4 travellers (2 adults, 2 children)`. | Both halves of the requirement are then true and nothing is invented: the child rate is published, and a 12-year-old is not quietly given a discount the rule does not offer. |
| refine-1 | R7: "every day block names at least one experience." | A day names every experience it has, and where the base town has run out it says `Nothing scheduled — this day is yours`. | R21 forbids dealing the same experience twice — that padding is what put the same morning on Day 6 and Day 7 — and R7 (amended) forbids scheduling anything more than 90 minutes from the booked base. A trip longer than its base town has things to do therefore has empty days, and saying so is the honest option. Trips of five nights or less still fill their days. |
| refine-1 | R19's example: 4 → 5 travellers swaps `Kanchenjunga View Retreat (₹8,600/night, premium)` for `Pelling Ridge Lodge (₹3,600/night, standard)`. | The notice has exactly that shape and wording, but the plan those Christmas-week dates now win is a different one, because R23's peak loading changed which candidate scores highest. | R23 and R19 landed in the same round; the literal Sikkim pair was true of the pre-seasonal engine. The behaviour the requirement is about — the swap is stated next to the total, with both names, both rates and both tiers — is asserted in `tests/reject-and-notice.test.ts` and `tests/round1-screens.test.tsx`. |
| refine-1 | R19: the overridden answer is named "in the same notice" as the stay swap, with the relaxation banner as its source. | The notice reads `Plan.why.reasons[].held` instead. | The case that produced the complaint — International answered with Manali — is the case where the ladder **exhausts** rather than relaxing one rung, so there is no `Relaxation` object at all and a banner-based notice would have stayed silent exactly when it mattered. `held` is set by the same code that writes the "we could not hold this answer" reason, so the two cannot disagree. |
| refine-1 | R23: the off-season line in `docs/05-customer-feedback.md` reads `-20%` with an ASCII hyphen. | `Off season (Jul): −20% on stay and travel`, with the PRD's U+2212 MINUS SIGN. | `01-prd.md` §10 is the acceptance spec QA extends from and it writes the minus sign; the feedback document is a transcript. Recorded because a substring search for `-20%` will not match. |
| refine-1 | R7 (amended): "every experience and stay gets a base town", implying a base per town named in a destination's title. | Two or three bases only where a **stay** genuinely sits in another town (`Kochi & Varkala`, `Gangtok & Pelling`, `Ella & the south coast`). Elsewhere one hub base, with `minutesFromBase` filtering out what cannot be reached from it — Cellular Jail (150), Mahabalipuram (120), Ha Long (150), the three Kathmandu-valley days (370+), Poon Hill (150) and Hanifaru (240) are in the catalogue and never scheduled. | Splitting every multi-town label into bases leaves some bases with three things to do, which trades a physically impossible itinerary for an empty one. The rule QA asserts — nothing scheduled more than 90 minutes from the stay's base, and the header naming that base — holds either way, and `tests/itinerary.test.ts` also requires every stay's base to have at least four reachable experiences. |
| refine-1 | R11: an absent alternative renders its sentence rather than an empty box. | Still true when one is absent. When **both** are absent, the two sentences are replaced by the single `Not this one — somewhere else` control. | R22 requires exactly that ("where neither a Saver nor a Stretch exists, the alternatives area renders one control instead of two empty slots"). `saverAbsentReason` and `stretchAbsentReason` are still computed and still persisted; the domain contract is unchanged. |
| refine-1 | §4.2 / R15: `PersistedSession` is schema 1. | Schema 2. A v1 blob is dropped, not migrated, and the user lands on the vibe screen with the defaults. | v1 knew nothing about adults, children, the free day or the rejected destinations, and it priced every traveller as an adult against a catalogue with no season. Guessing at the missing halves would produce a plan the user never asked for; R13 makes re-deriving free. |
| refine-1 | — (found while building) | `eslint.config.js` now ignores `.agency/**`. | The customer-judge scratch scripts committed under `.agency/panel/` are Node ESM files linted with browser globals, and `npm run lint` was already failing on 154 `no-undef` errors before this round's first edit. They are evidence, not product source. |
...
| F1 | §4.5: the relaxation ladder drops the highest-priority-number constraint first, stops at the first non-empty pool, and never reconsiders. | `survive()` (`domain/planner.ts`) now scores every affordable candidate by how many droppable constraints it actually fails, and the *global* minimum — the fewest concessions any real trip needs — decides the drop set, cheapest candidate at that minimum breaking ties. On top of that, `stableSurvive()` refuses to recommend a plan whenever restoring the dropped constraint would cost the same or less: it folds that constraint back in as mandatory and searches again, up to once per droppable constraint. | The fixed order could drop the wrong constraint (R14's own reproduction: dropped "city nightlife" when "a city" was the actual bottleneck) and, separately, `bestOf`'s score-based winner (§4.5, `WEIGHT.budgetFit` rewards spending nearer the budget) could land on a plan pricier than the cheapest option satisfying the very thing it dropped — the two together are what made `restore.costDelta` go negative. Picking the true minimal-violation set fixes the first; folding in a dominated constraint fixes the second. `tests/planner.test.ts` asserts `costDelta < 0` never occurs, swept across every vibe, region and a range of budgets and party sizes, plus the architecture review's own reproduction verbatim. |
| F1 | §3 invariant C3: "≥ 10 non-repeatable + ≥ 2 repeatable experiences **per destination**". | A second, re-based C3 in `tests/catalogue.test.ts`: `eligibleExperiences(destination, stay).length >= destination.maxNights + 1`, swept over all 42 `(destination × stay)` pairs. The original per-destination count is unchanged and still holds. | Fix 9 (refine-1) made the scheduler's real supply unit the *base town* a stay picks, not the destination, and nothing re-sized `maxNights` to match. 12 of 42 pairs failed the re-based check — `in-varkala` and `in-gangtok` (6 experiences per base, `maxNights` 14), `int-pokhara` (8, `maxNights` 16) and `int-ella`'s Mirissa base (4, `maxNights` 16) — plus eight more pairs that only narrowly missed it (11–12 experiences against `maxNights` 12–16: `in-goa`, `in-andaman`, `in-manali`, `int-bali`, `int-bangkok`, `int-phuket`, `int-hanoi`). Each was closed by lowering `maxNights` to `(eligible experiences − 1)` for that destination (content is unchanged; the offered trip length is honest about what the base can fill), except `int-ella`, where four new Mirissa experiences (`ella-weligama`, `ella-coconuthill`, `ella-stiltfisher`, `ella-harbourwalk`) bring that base up to parity with Ella's own 8, rather than capping the whole destination down to 3 nights. |
| F1 | R21: a day left empty prints `Nothing scheduled — this day is yours` whenever it has no experiences. | `buildDays` now names the day only when it is `itinerary.requestedFreeDay` (the day `ctx.freeDay` removed); any other empty day — caused by the base running out of supply — reads `supplyShortfallNote()`: `Nothing scheduled here — <base> has <n> things to do and this trip is <m> days`. `scheduleItinerary` also stopped zeroing the free day's *capacity* before dealing experiences (which only reduced demand, and did nothing on a supply-constrained base — B6) and instead deals to full capacity first, then deletes whatever landed on the chosen day outright, with no repacking onto other days. | R21's own (amended) acceptance criterion names both sentences and requires the total to strictly fall when the box is ticked; R7's requires the supply-shortfall line never claims the user's choice. Both are unit-tested per `(destination × stay)` pair and by an itinerary-level assertion that ticking the box always drops the total. |
| F4 | R8 (round-1 wording): "changing travellers from 2 to 4 increases the travel line item by exactly the per-person fare × 2", unconditionally (B3). | R8 is amended in `01-prd.md` §10 (A19): the property is asserted only for a re-plan that **keeps the same destination**. R9's stretch ceiling (never recommend above budget × 1.25) takes precedence; a headcount change that forces a destination switch is exempt from the linear-fare literal and is covered by the R19 change notice instead. `e2e/qa-03-plan.spec.ts`'s R8+UX13 test now branches on whether the destination changed. | QA (round 2) found the ₹60,000 reference budget produces exactly this case (Kochi & Varkala → Puducherry & Auroville) and every number shown was internally consistent — R9 was doing its job. B3 was a wording bug in the requirement, not the engine; this closes it by amending R8 rather than weakening R9, as QA recommended. |
| F4 | UX8: the heading after "International" must contain `long-haul`; the design doc's own copy table gave `How long a flight are you willing to sit through?`, which does not (B2). | The heading is now `Happy with a long-haul flight, or keep it short?` in both `src/domain/questions/graph.ts` and `03-design.md`'s S3 copy table. | The two documents disagreed (B2); the Tech Lead's review recommended fixing the copy rather than weakening UX8, since the word is what a user actually reads. The branching behaviour itself was already correct and untouched. |
| F4 | `.plan-hero__id` was one text node, `Plan <id> · catalogue <version>`, and could wrap mid-token at 360px (B4). | Rendered as two `<span className="plan-hero__id-token">` (each `white-space: nowrap`) joined by a plain `· ` text node, so the browser's only wrap point is between the two tokens. | Testing Library's `getNodeText` only reads an element's *direct* text-node children, so several unit tests that waited on `screen.findByText(/· catalogue 2026-08-01$/)` had to move to `waitFor(() => expect(document.querySelector('.plan-hero__id')?.textContent …` — a DOM-level read, unaffected by how many child nodes carry the text. Listed here because it is a real, if mechanical, cost of the fix. |
| F4 | `.dialog__text` (the export textarea) was `white-space: pre`, which never wraps and clipped every line at 360px (B5). | `white-space: pre-wrap` plus `overflow-wrap: break-word`. `03-design.md` §4 S6 updated to match. | `pre-wrap` preserves every character of the underlying value — including the newlines the export text depends on — while wrapping long lines visually; what gets copied to the clipboard is unchanged. |
| F4 | `.adjust__checkbox` (the free-day switch) was `width: 20px; height: 20px`, and `padding` added to grow the hit area had no effect on the measured box (B8). | `width: var(--tap-min); height: var(--tap-min)` (44×44) directly on the input. | A native `<input type="checkbox">`'s own box (what `getBoundingClientRect` — and UX22 — measure) did not grow for author `padding` in this build's Chromium, even though `box-sizing` and the padding value were both applied per `getComputedStyle`. Growing `width`/`height` directly is the only change that moved the measured box, confirmed with a standalone Playwright probe before landing it. |
